...
onMounted(async () => {
  ...
  await fetch("https://geo.api.gouv.fr/departements")
    ...
    .then((response) => {
      ...
      console.log("response liste des dpt", listeDep);
      // valeur 0 par defaut
      depSelect.valut = "0";
    })
    .catch((error) => console.log("erreur recup des dpt"));
});

const selectiond = async (dep) => {
  ...
  let request =
    ...
    dep +
    "&rows=50" +
    "&lang=FR&facet=departement_principal_de_deroulement";
  console.log({ request });
  await fetch(request)
    .then((response) => response.json())
    .then((response) => {
...
<template>
  <div>
    <div class="">
      <select
        class="my-4 mx-6 bg-black text-white rounded-lg"
        placeholder="selectionner un Département"
        v-model="depSelect"
        @change="selectiond(depSelect)"
      >
