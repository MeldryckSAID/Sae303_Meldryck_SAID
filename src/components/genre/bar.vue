...
onMounted(async () => {
  ...
  await fetch(request)
    ...
    .then((response) => {
      ...
      chartOptions.plugins.title.text += liste.value[0].nhits + " au total";
      ...
      liste.value[0].records.forEach((el) => {
        setLabels.add(el.fields.discipline_dominante);
      });
      ...
      chartData.labels.forEach((label) => {
        ...
        liste.value[0].records.forEach((reg) => {
          ...
          //on compte +1
//console.log("...",reg);
          if (label == reg.fields.discipline_dominante) {
            nb++
          }
        });
